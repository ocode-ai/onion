...
**Localização:** `${CLAUDE_PLUGIN_ROOT}/reference/docs/onion/` e `docs/onion/`
...
```markdown
1. Analise a documentação relevante (leia arquivos em ${CLAUDE_PLUGIN_ROOT}/reference/docs/onion/)
...
1. **Comandos** (`${CLAUDE_PLUGIN_ROOT}/commands/`) - Workflows executáveis
2. **Agentes** (`${CLAUDE_PLUGIN_ROOT}/agents/`) - Especialistas de IA
3. **Documentação** (`${CLAUDE_PLUGIN_ROOT}/reference/docs/onion/`) - Guias completos
...
Use formatação visual Unicode (conforme `${CLAUDE_PLUGIN_ROOT}/reference/docs/onion/clickup-integration.md`):
