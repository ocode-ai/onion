...
1. **Criar abstrações** - Implementar em `${CLAUDE_PLUGIN_ROOT}/reference/utils/clickup-mcp-wrappers.md`
...
5. **Documentar padrões** - Colocar em `${CLAUDE_PLUGIN_ROOT}/reference/docs/strategies/`
...
export async function updateSubtaskStatus(subtaskId: string, status: string) {
  ...
  if (!validStatuses.includes(status)) {
    throw new Error(
      `Status inválido: ${status}. Use um de: ${validStatuses.join(', ')}`,
    );
  }
